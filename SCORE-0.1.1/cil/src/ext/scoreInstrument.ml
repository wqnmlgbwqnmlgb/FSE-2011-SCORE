...
let shouldSkipFunction f = hasAttribute "score_skip" f.vattr
...
(* These definitions must match those in "libscore/score.h". *)
let idType   = intType
let bidType  = intType
...
let mkInstFunc f name args =
  ...
  let func = findOrCreateFunc f ("__Score" ^ name) ty in
    func.vstorage <- Extern ;
    func.vattr <- [Attr ("score_skip", [])] ;
    func
...
class scoreInstrumentVisitor f =
  (*
   * Get handles to the instrumentation functions.
   *
   * NOTE: If the file we are instrumenting includes "score.h", this
...
let addScoreInitializer f =
  let scoreInitFunc = mkInstFunc f "Init" [] in
  let globalInit = getGlobInit f in
    scoreInitFunc.vstorage <- Extern ;
    scoreInitFunc.vattr <- [Attr ("score_skip", [])] ;
    prependToBlock [mkInstCall scoreInitFunc []] globalInit.sbody
...
let feature : featureDescr =
  { fd_name = "ScoreInstrument";
    fd_enabled = ref false;
    fd_description = "instrument a program for use with SCORE";
    fd_extraopt = [];
    fd_post_check = true;
    fd_doit =
      function (f: file) ->
