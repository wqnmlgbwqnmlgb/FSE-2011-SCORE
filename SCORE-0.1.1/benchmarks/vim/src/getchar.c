...
vgetc()
{
#ifdef SCORE
    {
        ...
        if (++num_chars <= MAX_SYM_INPUTS) {
            char var_name[20];
            snprintf(var_name, sizeof(var_name), "input_char[%d]", num_chars);
            SCORE_int(c, var_name);
            return c;
        } else {
            exit(0);
            return 0;
...
int SCORE_getc(FILE* file) {
  ...
  char var_name[20];
  snprintf(var_name, sizeof(var_name), "input_char[%d]", num_chars);
  SCORE_unsigned_char(c, var_name);
  return c;
}
#endif  // SCORE
...
#else
        if (got_int || (c = SCORE_getc(scriptin[curscript])) < 0)
#endif
	{
	    closescript();
	    /*
