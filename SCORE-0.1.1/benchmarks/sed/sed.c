...
#ifdef SCORE
#include <score.h>
#define NUM_INPUT_LEN 148
int unget_flag = 0;
int unget_char = EOF;
int getc_(FILE *stream){
    static int num_chars = 0;
    unsigned char ch;
    if (stream == NULL) return EOF;
    if (++num_chars > NUM_INPUT_LEN){
        return EOF;
    }else if (unget_flag){
        unget_flag = 0;
        return unget_char;
    }else{
        SCORE_unsigned_char(ch);
    }
    return ch;

}
int ungetc_(int c, FILE *stream){
    if (stream == NULL) return EOF;
    else if (unget_flag != 0) return EOF;
    else{
        unget_char = c;
        unget_flag = 1;
        return unget_char;
