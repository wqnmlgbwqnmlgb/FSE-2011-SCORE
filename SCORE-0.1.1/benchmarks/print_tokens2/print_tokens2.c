...
int MAX = 82;
//unsigned char *sym_addr[BUFSIZ];
unsigned char sym_char[BUFSIZ];
int my_getc(FILE *stream){

    unsigned char ch;
    if (cur_pos == MAX){ return EOF;}
    if (cur_pos == max_pos){
        char var_name[10];
        snprintf(var_name, sizeof(var_name), "stdin[%d]", cur_pos);
        SCORE_unsigned_char(ch, var_name);
        sym_char[cur_pos] = ch;
        cur_pos++;
        max_pos++;
...
int my_ungetc(int c, FILE *stream){
    cur_pos--;
    sym_char[cur_pos] = c;
    return c;
}
...
static unget_error(character_stream *);
