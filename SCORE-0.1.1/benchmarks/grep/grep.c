...
{
  int nlines, i;
  size_t residue, save; //size_t==unsigned int
  char *beg, *lim;
     ...
     char *argv[];
{
  ...
  keys = NULL;  //tmp for each argv
  keycc = 0;	//size of cur keys
  keyfound = 0;	// indicate specific file of pattern
  count_matches = 0;	//
  no_filenames = 0;	//
  list_files = 0;	//
  suppress_errors = 0;	//
  ...
  while(opt_len>0)
#else
  while ((opt = getopt(argc, argv, "0123456789A:B:CEFGVX:bce:f:hiLlnqsvwxy")) != EOF)/*see _getopt_internal()*/
	/*updated: optind, nextchar,*/
	/* the sequence of paras in argv is rearranged after getopt, so optind means where options is finished, argv[optind] means the pattern, if pattern is not specified in -e or -f*/ 
#endif
  {
#ifdef SCORE
    char sym_optarg[2]={0,0};
    SCORE_int(opt, "opt");
    SCORE_char(sym_optarg[0], "sym_optarg");
    optarg = &sym_optarg[0];
    opt_len--;	
#endif
      switch (opt)
	...
	break;
      case 'A':
	out_after = atoi(optarg); /*convert optarg to number*/
	if (out_after < 0)
	  usage();
	break;
      case 'B':
...
#ifdef SCORE
{
	...
	for(i=0; i<keycc; i++){
        char var_name[10];
        snprintf(var_name, sizeof(var_name), "keys[%d]", i);
        SCORE_char(keys[i], var_name);
    }
}
#endif
      }
  ...
  if (optind < argc)
	...
	if (desc < 0)
	...
	else
	  {
	    ...
	    count = grep(desc);/*grep(int) is used to find match line in the file, and show matched lines*/ 
		fprintf(stderr, "count=%d, list_files=%d\n out_file=%d\n", count, list_files, out_file);
	    if (count_matches)
	      {
		if (out_file)
		  printf("%s:", filename);
...
int
getopt_internal (argc, argv, optstring, longopts, longind, long_only)
     int argc;
     char *const *argv;
